// v41 - carregamento determinístico: serviço -> SINAPI -> cronograma; físico-financeiro -> UI
(function(){
  ...
  function load(src,flag,msg){return new Promise((resolve,reject)=>{if(window[flag]){resolve();return}const s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=()=>{console.error(msg);reject(new Error(msg))};document.head.appendChild(s)})}
  async function boot(){
    try{
      await Promise.all([
        load('app-management-v30.js?v=41','__managementV30','Falha ao carregar painel de gestão'),
        load('app-compositions-v33.js?v=41','__compositionsV33','Falha ao carregar composições'),
        load('app-budget-export-v34.js?v=41','__budgetExportV34','Falha ao carregar exportação'),
        load('app-detail-composition-v37.js?v=41','__detailCompositionV37','Falha ao carregar composição na lista')
      ]);
      await load('app-physical-financial-v31.js?v=41','__physicalFinancialV31','Falha ao carregar físico-financeiro');
      await load('app-ui-v32.js?v=41','__uiV32','Falha ao carregar UI');
      await load('app-service-manager-v35.js?v=41','__serviceManagerV35','Falha ao carregar gerenciador de serviços');
      await load('app-sinapi-v38.js?v=41','__sinapiV38','Falha ao carregar busca SINAPI');
      await load('app-schedule-service-v39.js?v=41','__scheduleServiceV39','Falha ao carregar integração cronograma');
    }catch(e){console.error('Falha no carregamento complementar v41',e)}
  }
  boot();
})();