...
(function(){
  ...
  window.openBudgetMacroV29=function(macro){
    ...
    window.__detailFilters.q='';window.__detailFilters.macro=String(macro||'');window.__detailFilters.type='all';window.__detailFilters.status='all';window.__detailFilters.sort='code';
    renderBudget('detail');try{window.scrollTo({top:0,behavior:'smooth'})}catch(e){window.scrollTo(0,0)}
  };

  function decorateSummary(){
    const el=document.getElementById('budget');if(!el||window.__budgetTab!=='summary')return;
    const items=[...el.querySelectorAll('.list > .item')],macros=state.macros||[];
    items.forEach((item,i)=>{const m=macros[i];if(!m||item.dataset.drillV29)return;item.dataset.drillV29='1';item.classList.add('budget-summary-stage-v29');item.setAttribute('role','button');item.setAttribute('tabindex','0');item.addEventListener('click',()=>window.openBudgetMacroV29(m.name));item.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();window.openBudgetMacroV29(m.name)}});const grow=item.querySelector('.grow');if(grow&&!grow.querySelector('.budget-drill-v29'))grow.insertAdjacentHTML('beforeend',`<span class="budget-drill-v29">ver orçamento detalhado desta etapa ›</span>`)});
  }
  window.renderBudget=function(tab,month){const active=tab||window.__budgetTab||'summary';const out=previous(tab,month);if(active==='summary')setTimeout(decorateSummary,0);return out};
