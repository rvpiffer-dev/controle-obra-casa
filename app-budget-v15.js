// orçamento completo v16 - preserva aba selecionada durante renderizações/sincronização
(function(){
  ...
  window.renderBudget=function(tab,month){
    // Quando renderAll() chama renderBudget() sem argumentos, mantém a aba atual.
    tab=tab||window.__budgetTab||'summary';
    window.__budgetTab=tab;
    if(month) window.__budgetMonth=month;

    if(tab==='buy'&&old)return old('buy',month||window.__budgetMonth||undefined);
