// v35 - inclusao, retirada e reativacao de servicos do orcamento
(function(){
  ...
  style.textContent=`.budget-tools-v35{display:flex;gap:7px;justify-content:flex-end;flex-wrap:wrap;margin:8px 0}.removed-v35{opacity:.72}.service-danger-v35{margin-top:12px;border-top:1px solid var(--line);padding-top:12px}.service-form-grid-v35{display:grid;grid-template-columns:1fr 1fr;gap:8px}.service-form-grid-v35 .wide{grid-column:1/-1}@media(max-width:480px){.service-form-grid-v35{grid-template-columns:1fr}}`;
  ...
  window.openAddServiceV35=function(){
    const macros=state.macros||[];const first=macros[0]?.name||'';
    openModal(`<h2>Adicionar serviço</h2><div class="meta"><span>O código precisa ser único no orçamento.</span></div><div class="service-form-grid-v35"><div><label>Código</label><input id="asCode35" placeholder="Ex.: 05.13"></div><div><label>Unidade</label><input id="asUnit35" placeholder="un, m², m³, kg..."></div><div class="wide"><label>Nome do serviço</label><input id="asName35" placeholder="Descrição do serviço"></div><div class="wide"><label>Macroetapa</label><select id="asMacro35">${macroOptions()}</select></div><div class="wide"><label>Atividade do cronograma</label><select id="asTask35">${taskOptions()}</select></div><div><label>Quantidade</label><input id="asQty35" type="number" min="0" step="any" value="1"></div><div><label>Material unitário</label><input id="asMat35" type="number" min="0" step="0.01" value="0"></div><div><label>Mão de obra unitária</label><input id="asLab35" type="number" min="0" step="0.01" value="0"></div><div><label>Equip./Outros unitário</label><input id="asEO35" type="number" min="0" step="0.01" value="0"></div><div class="wide"><label>Código SINAPI / referência (opcional)</label><input id="asRef35" placeholder="Ex.: 96526"></div></div><div class="actions"><button class="btn alt" onclick="closeModal()">Cancelar</button><button class="btn" onclick="saveAddServiceV35()">Adicionar</button></div>`);
  };

  window.saveAddServiceV35=async function(){
    const code=String(document.getElementById('asCode35')?.value||'').trim(),name=String(document.getElementById('asName35')?.value||'').trim(),unit=String(document.getElementById('asUnit35')?.value||'').trim(),macro=String(document.getElementById('asMacro35')?.value||'').trim(),taskRaw=document.getElementById('asTask35')?.value||'',reference=String(document.getElementById('asRef35')?.value||'').trim();
    ...
    if(!/^\d{2}\.\d{2,3}$/.test(code)){alert('Use um código no padrão 05.13.');return}
    if(allStored().has(code)){alert('Já existe um serviço com o código '+code+'.');return}
    ...
    closeModal();await save();renderBudget('detail');
  };
  ...
  window.removeServiceV35=async function(code){
    ...
    const base=allStored().get(String(code))||s;
  ...
  window.reactivateServiceV35=async function(code){
    ...
    const originalQty=Number(s.qty)||1;o.qty=originalQty;closeModal();await save();renderBudget('detail');
  };
  ...
  function decorateTools(){
    ...
    const tabs=el.querySelector('.tabs');const tools=document.createElement('div');tools.className='budget-tools-v35';tools.innerHTML=`<button class="btn alt" onclick="openRemovedServicesV35()">Serviços retirados (${removedCodes().length})</button><button class="btn" onclick="openAddServiceV35()">＋ Adicionar serviço</button>`;
    if(tabs)tabs.insertAdjacentElement('afterend',tools);else el.prepend(tools);
  }
  window.renderBudget=function(tab,month){const active=tab||window.__budgetTab||'summary';const out=previousBudget(tab,month);if(active==='detail')setTimeout(decorateTools,0);return out};
